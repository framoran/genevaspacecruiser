...
<script>
	...
	cometImage.src = "{{ asset('/images/gros_rocher_special.png') }}"; // définit le chemin vers la source
...
this.timeStart = timeStart;
  ...
  function message() {
	...
	this.longueur = 150;
	this.color = "#01DF01";
	this.text = 'Appuyez sur "Barre Espace" dès que vous voyez';
	this.tx = 150;
	this.score1 = 0;
	...
	this.draw = function() {
		...
		if ((game.keyCode3 == true) && (myGamePiece_Affichage.keypress == false) && (myGamePiece_DrawingComet.cometX < 1000 && myGamePiece_DrawingComet.cometX > 0)) {
		    ...
		    setTimeout("keyRestore()", 3000);
			keyRestore = function(){
				myGamePiece_Affichage.keypress = false;
			};
			myGamePiece_Affichage.keypress = false;
			if (game.update < 1){
				myGamePiece_Affichage.text = 'Appuyez à nouveau sur "Barre Espace" dès que vous voyez';
				myGamePiece_Affichage.tx = 150;
				game.update = 1;
				myGamePiece_Affichage.keypress = true;
			}else if (!myGamePiece_Affichage.keypress){
				game.update = 2;
...
function updateGame() {
	...
	context.fillText("un rocher spécial pour faire le plein de carburant.", 150, 300);
